module Model.Date exposing (Date, Month(..), compare, compareMonth, full, month, monthToString, monthsBetween, monthsBetweenMonths, offsetMonths, onlyYear, view, year, returnYear, returnMonth)
...
monthsBetween dA dB =
    if ((checkMonth (month dA) < 12) && (checkMonth (month dB) < 12)) then
        if ((year dA) > (year dB)) then
            if (checkMonth (month dA) < checkMonth (month dB)) then
                Just (abs ((year dA - year dB) * 12) - abs (checkMonth (month dA) - checkMonth (month dB)))
            else 
                Just (abs ((year dA - year dB) * 12) + abs (checkMonth (month dA) - checkMonth (month dB)))
    ...
    else if ((checkMonth (month dA) == 12) && (checkMonth (month dB) == 12)) then
...
-- helper function, returns the respective integer if the month is defined, or 12 if it's Nothing
checkMonth : Maybe Month -> Int
checkMonth m = 
            case m of
                Just Jan -> monthToInt Jan
                Just Feb -> monthToInt Feb
                Just Mar -> monthToInt Mar
                Just Apr -> monthToInt Apr
                Just May -> monthToInt May
                Just Jun -> monthToInt Jun
                Just Jul -> monthToInt Jul
                Just Aug -> monthToInt Aug
                Just Sep -> monthToInt Sep
                Just Oct -> monthToInt Oct
                Just Nov -> monthToInt Nov
                Just Dec -> monthToInt Dec
                Nothing -> 12
...
compare (Date d1) (Date d2) =
    if (d1.year < d2.year) then
        ...
        GT
    else if (checkMonth d1.month > checkMonth d2.month) then
        GT
    else if (checkMonth d1.month < checkMonth d2.month) then
        LT
    else 
...
view (Date d) =
    let
        maybeMonthToString m = 
            case m of
                Just Jan -> monthToString Jan
                Just Feb -> monthToString Feb
                Just Mar -> monthToString Mar
                Just Apr -> monthToString Apr
                Just May -> monthToString May
                Just Jun -> monthToString Jun
                Just Jul -> monthToString Jul
                Just Aug -> monthToString Aug
                Just Sep -> monthToString Sep
                Just Oct -> monthToString Oct
                Just Nov -> monthToString Nov
                Just Dec -> monthToString Dec
                Nothing -> ""
    in
    
    div [] [
        p [] [text <| String.fromInt d.year ++ "-" ++ maybeMonthToString d.month]
    ]
...
returnYear : Date -> Int
returnYear (Date date) =
    date.year

returnMonth : Date -> String
returnMonth (Date date) = 
    case date.month of
        Just Jan -> monthToString Jan
        Just Feb -> monthToString Feb
        Just Mar -> monthToString Mar
